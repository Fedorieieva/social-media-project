        ...
        .then((user) => {
            if (user) {
                ...
                if (user.login === req.body.login) {
                    return res.status(400).json({message: `Login ${user.login} already exists`});
                }
            }

            // Create query object for user for saving them to DB
                    ...
                    newUser
                        ...
                        .then((user) => {
                            ...
                            jwt.sign(
                                payload,
                                keys.secretOrKey,
                                {expiresIn: 36000}, // You can adjust the expiry time as needed
                                (err, token) => {
                                    if (err) {
            ...
            if (req.body.email) {
                ...
                if (currentEmail !== newEmail) {
                    User.findOne({email: newEmail}).then((user) => {
                        if (user) {
                            errors.email = `Email ${newEmail} already exists`;
            ...
            if (req.body.login) {
                ...
                if (currentLogin !== newLogin) {
                    User.findOne({login: newLogin}).then((user) => {
                        if (user) {
                            errors.login = `Login ${newLogin} already exists`;
                            res.status(400).json(errors);
                            return;
                        }
                    });
